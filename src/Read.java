...
public class Read {
	...
	public void readFormat(Model _pModel, int _pOut) {

		if (_pOut == 1)
			_pModel.write(System.out, "RDF/XML");
		
		if (_pOut == 2)
			_pModel.write(System.out, "JSON-LD");
		
		if (_pOut == 3)
			//_pModel.write(System.out, "TTL");
			RDFDataMgr.write(System.out, _pModel, RDFFormat.TURTLE_PRETTY);
	...
	public Model readAll(Model _pModel) {
		...
		// fetch all names
		String _queryString = "PREFIX foaf: <http://xmlns.com/foaf/0.1/> " + "SELECT ?name ?mbox WHERE { "
				+ "    ?person foaf:mbox ?mbox . " + "    ?person foaf:name ?name . " + "}";
		...
		while (_rs.hasNext()) {
			...
			String _queryStringKnow = "PREFIX foaf: <http://xmlns.com/foaf/0.1/> " + "SELECT ?know WHERE { "
					+ "    ?person foaf:name \"" + _name + "\" . " + "    ?person foaf:knows ?know . " + "}";
